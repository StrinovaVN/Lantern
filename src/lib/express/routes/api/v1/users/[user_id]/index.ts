...
export const get = [
  ...
  async (request: Request<RequestParams, unknown, unknown, APIUsersGETRequestQuery>, response: Response) => {
    ...
    if (svg == 1) {
      const stringifyOption = (value: unknown): string | undefined => {
        if (value === null || value === undefined) return undefined;

        return String(value);
      };

      const options: APIUsersGETRequestQuery = {
        theme: request.query.theme,
        borderRadius: stringifyOption(request.query.borderRadius),
        hideGlobalName: stringifyOption(request.query.hideGlobalName),
        hideStatus: stringifyOption(request.query.hideStatus),
        hideBadges: stringifyOption(request.query.hideBadges),
        hideActivity: stringifyOption(request.query.hideActivity),
        hideLastSeen: stringifyOption(request.query.hideLastSeen),
        hideServerTag: stringifyOption(request.query.hideServerTag),
        noActivityTitle: request.query.noActivityTitle,
        noActivityMessage: request.query.noActivityMessage
      };
