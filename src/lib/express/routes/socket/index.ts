...
async function subscribeToUsers(socket: WebSocket, socketId: string, { user_id, user_ids }: { user_id?: string, user_ids?: string[] }) {
  ...
  if (ActiveSockets.has(socketId)) {
    ...
    ActiveSockets.set(socketId, {
      ...
      subscribed: subscribed_to_all ? 'ALL' : [...subscribed, ...usersToSubscribe]
    });

    send(socket, Opcodes.SUBSCRIBE_ACK);

    logger.log('socket', `Websocket connection ${socketId} subscribed to ${subscribed_to_all ? 'all users' : usersToSubscribe.join(', ')}.`);
  } else {
    const usersToSubscribe = user_ids || (user_id ? [user_id] : []);
    ...
    ActiveSockets.set(socketId, {
      instance: socket,
      lastHeartbeat: Date.now(),
      subscribed: subscribed_to_all ? 'ALL' : usersToSubscribe
    });
  }
}
    ...
    websocket.on('message', async message => {
      ...
      switch (op) {
        ...
        case Opcodes.HEARTBEAT: {
          ...
          const heartbeatSocket = ActiveSockets.get(id);
          if (!heartbeatSocket) return disconnect(websocket, null, 'Invalid websocket connection.');
        ...
        case Opcodes.SUBSCRIBE: {
          ...
          const subscribedSocket = ActiveSockets.get(id);
          if (!subscribedSocket) return disconnect(websocket, null, 'Invalid websocket connection.');

          var error = getZodError(data, InitSchema);
          if (error) return send(websocket, Opcodes.ERROR, error);

          // if already subscribed to all users, return an error.
          if (subscribedSocket.subscribed === 'ALL') return send(websocket, Opcodes.ERROR, 'You are already subscribed to all users.');
        ...
        case Opcodes.UNSUBSCRIBE: {
          ...
          const unsubscribedSocket = ActiveSockets.get(id);
          if (!unsubscribedSocket) return disconnect(websocket, null, 'Invalid websocket connection.');

          var error = getZodError(data, InitSchema);
          if (error) return send(websocket, Opcodes.ERROR, error);

          var { user_id, user_ids } = data;
          var { subscribed } = unsubscribedSocket;
